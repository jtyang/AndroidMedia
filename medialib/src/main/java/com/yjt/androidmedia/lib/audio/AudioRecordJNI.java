...
package com.yjt.androidmedia.lib.audio;
...
import com.yjt.androidmedia.lib.utils.XLog;

import java.nio.ByteBuffer;


/**
 * source from:https://github.com/DrKLO/Telegram/blob/master/TMessagesProj/src/main/java/org/telegram/messenger/voip/AudioRecordJNI.java
 */
public class AudioRecordJNI {
    ...
    private boolean tryInit(int source, int sampleRate) {
        ...
        XLog.d("Trying to initialize AudioRecord with source=" + source + " and sample rate=" + sampleRate);
        ...
        try {
            ...
            XLog.e("AudioRecord init failed!", x);
        }
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                    try {
                        if (AutomaticGainControl.isAvailable()) {
                            ...
                            XLog.w("AutomaticGainControl is not available on this device :(");
                        }
                    } catch (Throwable x) {
                        XLog.e("error creating AutomaticGainControl", x);
                    }
                    try {
...
//									ns.setEnabled(VoIPServerConfig.getBoolean("user_system_ns", true));
                            ...
                            XLog.w("NoiseSuppressor is not available on this device :(");
                        }
                    } catch (Throwable x) {
                        XLog.e("error creating NoiseSuppressor", x);
                    }
                    try {
...
//									aec.setEnabled(VoIPServerConfig.getBoolean("use_system_aec", true));
                            ...
                            XLog.w("AcousticEchoCanceler is not available on this device");
                        }
                    } catch (Throwable x) {
                        XLog.e("error creating AcousticEchoCanceler", x);
                    }
                }
                startThread();
            } else {
            ...
            XLog.e("Error initializing AudioRecord", x);
        }
        return false;
    }
